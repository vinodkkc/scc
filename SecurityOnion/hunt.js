...
const huntComponent = {
  ...
  data() { return {
    ...
    soarUrl: null,
    soarHiveUsername: null,
    soarHivePassword: null,
    hostDomainName: null,
    customEscalateOptionVisible: false,
  ...
  methods: {
    ...
    initHunt(params) {
      ...
      for (let action of this.actions) {
        if (action.name == "acdsSoarURL") {
          this.soarUrl = action.links[0];
          this.soarHiveUsername = action.links[1];
          this.soarHivePassword = action.links[2];
        }
      }
    ...
    async loadData() {
      ...
      try {
        ...
        if (response.data.metrics["bottom"] != undefined) {
          this.customEscalateOptionVisible = false;
          ...
          this.customEscalateOptionVisible = true;
        }
        this.loaded = true;
        this.expanded = [];
    ...
    async ack(event, item, idx, acknowledge, escalate = false, caseId = null, hiveCase = false, hiveAlert = false) {
      this.$root.startLoading();
      try {
        var docEvent = item;
        if (item["soc_id"]) {
        ...
        if (escalate) {
          if (!caseId || !this.escalateRelatedEventsEnabled) {
            ...
            var hiveVar = this.buildCase(item);
            hiveVar["severity"] = parseInt(hiveVar["severity"]);
            hiveVar["template"] = null;
            hiveVar["title"] = item["log.full"];
            hiveVar["tags"] = item["tags"];
            var artifact = [];
            var artifactData = 'agent.ip' in item && item['agent.ip'] ? '' + item['agent.ip'] : null;
            var artifactData2 = 'metadata.ip_address' in item && item['metadata.ip_address'] ? '' + item['metadata.ip_address'] : null;
            var artifactData3 = 'destination.ip' in item && item['destination.ip'] ? '' + item['destination.ip'] : null;
            this.createArtifact(artifactData) ? artifact.push(this.createArtifact(artifactData)) : null;
            this.createArtifact(artifactData2) ? artifact.push(this.createArtifact(artifactData2)) : null;
            this.createArtifact(artifactData3) ? artifact.push(this.createArtifact(artifactData3)) : null;
            ...
            if (hiveCase) {
              var hive_observable = {};
                ...
                const case_response = await this.$root.papi.post(this.soarUrl + 'api/case', hiveVar,
                                      {
                                        auth:{username: this.soarHiveUsername, password: this.soarHivePassword},
                                      });

                const case_observable_response = await this.$root.papi.post(this.soarUrl + 'api/case/' + case_response.data.id + '/artifact', hive_observable,
                                                 {
                                                   auth:{username: this.soarHiveUsername, password: this.soarHivePassword},
                                                 });
            }
            ...
            if (hiveAlert) {
              ...
              hiveVar["type"] = "Security Onion";
              hiveVar["source"] = item["soc_source"];
              hiveVar["sourceRef"] = item["soc_id"];
              hiveVar["externalLink"] = this.hostDomainName + "#/alerts?q=_id%3A" + item['soc_id'] + "%20AND%20event.dataset%3Aalert%20AND%20event.acknowledged%3Atrue%20AND%20event.escalated%3Atrue";
              hiveVar["artifacts"] = artifact;

              const alert_response = await this.$root.papi.post(this.soarUrl + 'api/alert', hiveVar,
                                     {
                                       auth:{username: this.soarHiveUsername, password: this.soarHivePassword},
                                     });
            }
    ...
    buildCurrentRoute() {
    	...
    	if(this.$route.query.embed){ 
    		isEmbeded = true; 
    		return { path: this.category, query: {embed:isEmbeded,q: this.query, t: this.dateRange, z: this.zone, el: this.eventLimit, gl: this.groupByLimit }}; 
    	}else{ 
    ...
    toggleEscalationMenu(domEvent, event) {
      this.hostDomainName = domEvent.srcElement.baseURI.split("#")[0];
      if (!this.escalateRelatedEventsEnabled) {
