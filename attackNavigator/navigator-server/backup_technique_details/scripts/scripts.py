...
class ExportToJson:
    ...
    #         temp_script_list = []
    #         for d_id in val:
    #             # print(detection_df[detection_df['detection_id'] == d_id]['script'].values)
    ...
    def detection_json(self):
        ...
        #             else:
        #                 tech_dec_map[i].append(j.detection_id)
                    # tech_dec_map {"T1212" = [2,1,5,6] }// detection ids
        # print(tech_dec_map.keys())
        # for index, row in tec_dec_df.iterrows():
        ...
        ######################################################
            # print("--#-> ", temp_tech_id)
            temp_tid = technique_df[technique_df['combine_tech_id'] == temp_tech_id]['tactic_id'].iloc[0]
            # print("==> ", temp_tid)
            # print("---> ", Technique.objects.filter(combine_tech_id=i).technique_name)
        ...
        for i in ds_list:
            if i:
                ...
                for j in i.split(","):
                    j = j.strip('"')
                    # print(j)
                    if j not in unique_ds_list:
                        unique_ds_list.append(j)

        # print(unique_ds_list)
